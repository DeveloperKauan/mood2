...
// LISTA COMPLETA DE FRASES (RESTAURADA E CORRIGIDA)
const QUOTES_DB = [
...
const calculateLifeWeeks = (birthDateString, maxYears = 80) => {
  ...
  return { lived: diffWeeks, total: maxYears * 52 };
};
...
// Cálculo de Nota e Média do Mês
...
// Gerador de Imagem do Heatmap (Canvas)
const generateShareImage = async (entries, userName, isDark) => {
    ...
    const emptyCellColor = isDark ? '#1e293b' : '#e2e8f0';
...
const StorageService = {
  ...
  saveData: (data) => {
    try { localStorage.setItem(STORAGE_KEY_DATA, JSON.stringify(data)); } catch (e) { console.error(e); }
  },
  loadSettings: () => {
    try {
...
const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const checkNotification = () => {
      ...
      if (currentTime === settings.notificationTime && lastNotified !== todayDate) {
        new Notification("Hora de registrar!", {
          body: "Como foi o seu dia? Registre seu humor agora.",
          icon: "/icon-192.png" 
        });
...
const Button = ({ children, onClick, variant = 'primary', className = '', ...props }) => {
  const baseStyle = "px-3 py-2 rounded-xl text-sm font-medium transition-all active:scale-95 flex items-center justify-center gap-2";
  const variants = {
...
const Card = ({ children, className = '', title }) => (
  <div className={`bg-white dark:bg-slate-800 p-4 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700 ${className}`}>
    {title && <h3 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">{title}</h3>}
    {children}
  </div>
);
...
const Modal = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-3xl p-5 shadow-2xl animate-in zoom-in-95 duration-200 relative max-h-[90vh] overflow-y-auto">
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Registro: ${dateKey}`}>
      <div className="space-y-4">
        <div>
          <label className="text-xs font-medium text-slate-500 mb-1 block">Como você está?</label>
          <div className="grid grid-cols-3 gap-2">
            {Object.entries(MOODS_CONFIG).map(([key, config]) => (
              <button
                key={key}
                onClick={() => setMood(key)}
                className={`p-2 rounded-xl border-2 transition-all flex flex-col items-center gap-0
                  ${mood === key 
                    ? `${config.color} border-transparent text-white scale-105 shadow-md` 
                    : 'border-slate-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-700'}`}
              >
                <span className="text-xl">{config.icon}</span>
                <span className={`text-[10px] font-bold ${mood === key ? 'text-white' : 'text-slate-500'}`}>{config.label}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <label className="text-xs font-medium text-slate-500 mb-1 block">O que rolou?</label>
          ...
          <textarea
            className="w-full p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border-none focus:ring-2 focus:ring-emerald-500 min-h-[80px] text-sm text-slate-700 dark:text-slate-200 placeholder-slate-400 resize-none"
...
/* ==================================================================================
   6. PAGE COMPONENTS (COMPACT MODE)
   ================================================================================== */

const Dashboard = () => {
  const { entries, settings, setView } = useContext(AppContext);
  ...
  const handleShare = async () => {
    try {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({
                files: [file],
                title: 'Meu Mês',
                text: `Resumo do mês! Nota: ${monthStats.grade}`,
            });
        } else {
    ...
    } catch (error) { console.error(error); alert("Erro ao gerar imagem."); }
  };
    ...
    <div className="space-y-4 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex flex-col items-end">
           <div className="flex items-center gap-1 text-emerald-500 font-bold bg-emerald-50 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full text-sm">
             <TrendingUp size={14} />
      ...
      {/* Quote Compacto */}
      <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl p-4 text-white shadow-lg shadow-emerald-500/20 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full blur-2xl -mr-10 -mt-10"></div>
        <Award className="w-6 h-6 text-emerald-200 mb-2 opacity-80" />
        <p className="text-sm font-medium leading-relaxed mb-2">"{quote.text}"</p>
        <p className="text-xs text-emerald-100 font-semibold opacity-80">— {quote.author}</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        <Card title="Tendência (7d)">
           <div className="h-14 w-full -ml-2 mb-2">
           ...
           <div className="flex justify-between items-center border-t border-slate-100 dark:border-slate-700 pt-2">
              <div>
                  ...
                  <p className={`text-xl font-bold ${monthStats.average >= 3 ? 'text-emerald-500' : 'text-rose-400'}`}>
                      {monthStats.grade}
                  </p>
              </div>
        ...
        <div className="flex flex-col gap-2">
          <button 
            ...
            className="flex-1 bg-slate-100 dark:bg-slate-800 rounded-2xl p-3 flex flex-col items-center justify-center gap-1 border-2 border-dashed border-slate-300 dark:border-slate-700 hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-slate-700 transition-all group"
          >
            <div className="bg-white dark:bg-slate-600 p-1.5 rounded-full shadow-sm group-hover:scale-110 transition-transform">
              {hasLoggedToday ? <span className="text-lg">{MOODS_CONFIG[entries[todayKey].mood].icon}</span> : <Plus className="w-4 h-4 text-emerald-500" />}
            </div>
          ...
          <Button variant="secondary" onClick={handleShare} className="!py-2 !rounded-xl !text-[10px]">
      ...
      <div onClick={() => setView('memento')} className="cursor-pointer">
        <Card title="Vida Vivida (Toque para ver)" className="relative overflow-hidden hover:border-emerald-500 transition-colors group">
            <div className="flex items-center gap-3">
                ...
                <span className="font-bold text-slate-800 dark:text-white text-xs">{lifePercentage}%</span>
            </div>
        </Card>
      </div>
...
const CalendarPage = () => {
  ...
  const renderDays = () => {
    const daysArray = [];
    for (let i = 0; i < firstDay; i++) daysArray.push(<div key={`empty-${i}`} className="aspect-square" />);
    for (let i = 1; i <= days; i++) {
      ...
      daysArray.push(
        ...
        >
          <span className={`text-[10px] font-medium z-10 ${entry ? 'text-white' : 'text-slate-400'}`}>{i}</span>
          {entry && <span className="absolute text-sm opacity-20">{MOODS_CONFIG[entry.mood].icon}</span>}
        </button>
      );
    }
    return daysArray;
    ...
    <div className="space-y-4 pb-20">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-1">
          <Button variant="secondary" className="!p-1.5 !rounded-lg" onClick={prevMonth}><ChevronLeft size={16}/></Button>
          <Button variant="secondary" className="!p-1.5 !rounded-lg" onClick={nextMonth}><ChevronRight size={16}/></Button>
        </div>
      </div>
...
const MementoPage = () => {
  ...
  return (
    <div className="pb-20 space-y-3 h-full flex flex-col">
      <div className="bg-slate-900 text-white p-4 rounded-3xl shrink-0">
        <h2 className="text-xl font-bold mb-1">Memento Mori</h2>
      ...
      <div className="flex-1 min-h-0 bg-white dark:bg-slate-800 rounded-3xl p-2 border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col items-center justify-center">
        <div 
          className="grid grid-cols-[repeat(52,1fr)] gap-[0.5px] w-full max-w-[400px] aspect-[52/80]"
        ...
        >
          {Array.from({ length: totalWeeks }).map((_, i) => {
             ...
             return (
               <div 
                 key={i}
                 className={`rounded-[0.2px] w-full h-full
      ...
      <p className="text-center text-[10px] text-slate-400 italic shrink-0">1 ponto = 1 semana</p>
    </div>
  );
};
    ...
    <div className="pb-20 space-y-4">
      ...
      <Card title="Preferências">
        <div className="space-y-3">
          <div className="flex justify-between items-center">
             <span className="text-sm text-slate-700 dark:text-slate-300 font-medium">Modo Escuro</span>
             <button 
               onClick={() => handleChange('theme', settings.theme === 'dark' ? 'light' : 'dark')}
               className="p-1.5 bg-slate-100 dark:bg-slate-700 rounded-full text-slate-600 dark:text-slate-200"
             >
          ...
          <div className="flex justify-between items-center" onClick={requestNotify}>
             <div className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
               <Bell size={16} />
               <span className="text-sm font-medium">Notificar Diariamente</span>
             </div>
             <input 
               type="time" 
               value={settings.notificationTime || ''}
               onChange={(e) => handleChange('notificationTime', e.target.value)}
               className="bg-slate-50 dark:bg-slate-900 border-none rounded-lg px-2 py-1 text-sm text-slate-700 dark:text-white focus:ring-2 focus:ring-emerald-500"
             />
          </div>
          ...
          <div>
            ...
            <input 
              ...
              className="w-full bg-slate-50 dark:bg-slate-900 border-none rounded-xl px-3 py-2 text-sm text-slate-700 dark:text-white focus:ring-2 focus:ring-emerald-500"
            />
          </div>
        </div>
      </Card>
      ...
      <Card title="Dados">
        <div className="space-y-2">
            <Button variant="secondary" onClick={exportData} className="w-full !text-xs !py-2">
            ...
            <input 
                ...
                style={{ display: 'none' }} 
            />
            <Button variant="secondary" onClick={handleImportClick} className="w-full !text-xs !py-2">
      ...
      <div className="text-center text-[10px] text-slate-400 mt-4">
        Stat Of Mind v2.5 • Full Features
      </div>
    </div>
  );
};
...
const BottomNav = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-950/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 pb-safe z-50">
      <div className="flex justify-between items-center max-w-md mx-auto h-14 px-6">
        {navItems.map(item => {
          ...
          return (
            ...
            >
              <item.icon 
                ...
                className={`mb-0.5 transition-all ${isActive ? 'scale-110' : 'scale-100'}`}
              />
